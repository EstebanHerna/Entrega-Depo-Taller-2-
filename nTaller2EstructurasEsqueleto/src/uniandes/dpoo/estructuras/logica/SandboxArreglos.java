...
public class SandboxArreglos
{
    ...
    public void eliminarCadena(String cadena) {
        List<String> cadenasNoEliminadas = new ArrayList<>();

        for (String elemento : arregloCadenas) {
            if (!elemento.equals(cadena)) {
                cadenasNoEliminadas.add(elemento);
            }
        }

        arregloCadenas = cadenasNoEliminadas.toArray(new String[0]);
    }
    /**
...
public void eliminarEnteroPorPosicion(int posicion) {
    if (posicion >= 0 && posicion < arregloEnteros.length) {
        ...
        System.arraycopy(arregloEnteros, posicion + 1, nuevoArreglo, posicion, nuevaLongitud - posicion);
